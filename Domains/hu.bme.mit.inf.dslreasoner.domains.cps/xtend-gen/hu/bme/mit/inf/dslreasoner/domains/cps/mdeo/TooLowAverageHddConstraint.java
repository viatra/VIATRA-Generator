...
public class TooLowAverageHddConstraint implements IGuidanceFunction {
  private static final double THRESHOLD = 0.25;
  
  @Override
  public String getName() {
    return "TooLowAverageHdd";
  }
  ...
  public double computeFitness(final Solution solution) {
    double _xblockexpression = (double) 0;
    {
      ...
      for (final HostType hostType : _hostTypes) {
        {
          ...
          for (final HostInstance host : _instances) {
            {
              final int utilization = ResourceUtilizationUtil.getHddUtilization(host);
              double _sumUtilization = sumUtilization;
              int _defaultHdd = hostType.getDefaultHdd();
              double _divide = (((double) utilization) / _defaultHdd);
      ...
      final double difference = (TooLowAverageHddConstraint.THRESHOLD - averageUtilization);
