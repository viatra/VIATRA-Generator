...
class RestoreFuzzTest {
	...
	@ParameterizedTest(name = "Restore Smoke {index}/{0} Steps={1} Keys={2} Values={3} commit frequency={4} seed={5} evil-hash={6}")
	@MethodSource
	@Timeout(value = 10)
	@Tag("smoke")
	...
	static Stream<Arguments> parametrizedSlowSmoke() {
		return FuzzTestUtils.changeStepCount(RestoreFuzzTest.parametrizedFastSmoke(), 1);
	}
}
