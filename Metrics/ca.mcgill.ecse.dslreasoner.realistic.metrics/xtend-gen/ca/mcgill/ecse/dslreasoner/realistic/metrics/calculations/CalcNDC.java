...
public class CalcNDC extends CalcMetric2 {
  ...
  public List<Double> calcFromModel(final EObject model) {
    final List<Double> ndaDistrib = CalcNDC.NDACalculator.calcFromModel(model);
    ...
    for (final Double nda : ndaDistrib) {
      {
        double metVal = 0.0;
        if (((nda).doubleValue() != 0)) {
          metVal = ((nda).doubleValue() / numNodes);
        }
        metricDistrib.add(Double.valueOf(metVal));
      }
    }
  ...
  public List<Double> calcFromNHLattice(final PartialInterpretation pm, final Integer depth) {
    ...
    final List<Double> ndaDistrib = CalcNDC.NDACalculator.calcFromNHLattice(nhDeepRep, nhRep, depth);
