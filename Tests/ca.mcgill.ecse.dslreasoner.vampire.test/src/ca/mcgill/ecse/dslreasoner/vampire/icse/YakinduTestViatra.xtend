...
//		val queries = null
		...
		for (var i = SZ_BOT; i <= SZ_TOP; i += INC*index) {
			index*=2
			var writer = new PrintWriter(
					dataWorkspace.workspaceURI + "//_viatra" + i + "x" + REPS + "-" + formattedDate + ".csv")
...
//					val modTime = (solution.statistics.entries.filter[name.contains("Answer")].
//						get(0) as StringStatisticEntry).value
				val modTime = solution.statistics.solverTime / 1000.0
